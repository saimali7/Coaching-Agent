...
**The mic is push-to-talk, and that is a product decision, not a control style.** Hold the button — or the space bar — speak, release. The WebSocket stays open between turns so there is no per-question handshake and the agent keeps the thread; only the microphone opens and closes. Between turns the agent hears nothing: not the gym, not the audience, not a spotter's joke it might mistake for an instruction. The REST gate is enforced three ways in `useCoachAgent.ts` (a `canTalk` guard, a bail-and-log in the hold handler, and an effect that ends the session the moment REST is left, demo jumps included). The demo rig can lift the gate for free-form conversation and labels that state as off-spec, because it is.
...
**One voice, two surfaces, one switch.** Because the coach speaks through both the agent and the pre-rendered cues, changing the voice is a single action that must move both: the demo rig's voice picker (and the `POST /api/voice/voice` + `POST /api/voice/cues/regenerate` pair behind it) repoints the ElevenLabs agent and re-renders all 27 cues in the same pass, writing into `public/cues` and the served build. Cue URLs carry a render version for cache busting, and the rig warns out loud when the two surfaces have drifted apart. The current default is **Coach Akeem** (`ELEVENLABS_VOICE_ID` in `.env`), a motivational fitness-coach voice rather than a narrator. One API wrinkle worth recording: English agents reject the multilingual `_v2_5` TTS models, so the agent runs `eleven_flash_v2` while the pre-rendered cues use `eleven_turbo_v2_5` — different endpoints, different constraints.
...
Post-close, demo-driven: push-to-talk with the space bar as the hold key · the rig's voice picker with live cue re-rendering · a screen wake lock for the length of a session (a browser tab loses audio when the phone locks, so keeping the screen awake is the honest web version of "put the phone down") · ngrok serving through the API's single origin, because `getUserMedia` needs HTTPS and the mic is the product.
